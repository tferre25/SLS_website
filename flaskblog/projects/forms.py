...
class ProjectForm(FlaskForm):  
    ...
    laboratories = SelectField("Laboratoire", choices=laboratories)# if username = 'Laboratoire diagnostic St Louis
    if_no_laboratory = StringField('Précisez quel laboratoire:',render_kw={'placeholder': 'cardio, reanimation...'}) # if laboratories = 'autre'
    clinical_service = StringField('Service',render_kw={'placeholder': 'cardio, reanimation...'}) # if username= Service clinique St Louis
    email = StringField('E-mail (personne en copier)', validators=[Email()],render_kw={'placeholder': 'firstName.lastName@aphp.fr'})
    ...
    urgency_of_request = SelectField('Urgence de la demande', choices=[('Not urgent', 'Non urgent'),
    ...
    if_regulatory_requirements = SelectField('Si des exigences réglementaires sont disponibles, veuillez préciser', choices=[('Other', 'Autres'),
