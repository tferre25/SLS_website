...
class ProjectForm(FlaskForm):  
    ...
    ##
    project_title = StringField('Project title :*', validators=[DataRequired(), Length(min=2)],
    ...
    organism = StringField('Organism (funding succeptible) :*', validators=[DataRequired(), Length(min=2)],
                           render_kw={'placeholder': 'Structure or organization funding the project'})
    
    principal_investigator = StringField('Principal investigator :*', validators=[DataRequired(), Length(min=2)],
                                         render_kw={'placeholder': 'first and last name'})
    
    promotor = StringField('Promotor :*', validators=[DataRequired(), Length(min=2)],
    ...
    if_urgency = TextAreaField('If very urgent, briefly explain the issues :*',
                               render_kw={'placeholder': 'Explain the reason for the emergency, depending on what has been selected from the drop-down list'})
    ##
    project_context = TextAreaField('Project context :*', validators=[DataRequired(), Length(min=10)],
    ...
    project_context_private = TextAreaField('Project context private:*', validators=[DataRequired(), Length(min=10)],
                                    render_kw={'placeholder': 'General explanations of the context of the project (ex : explain the rationale for the project, previous experiences or publications, general explanations of the disease, etc.)'})
    project_summary = TextAreaField('Project summary :*', validators=[DataRequired(), Length(min=10)],
    ...
    data_type = StringField('Data type :*', validators=[DataRequired(), Length(min=2)],
                            render_kw={'placeholder': 'ex: fastq, tables de comptage, bam, etc.'})
    data_size = FloatField('Data size(GO) :*', validators=[validators.InputRequired(), validators.NumberRange(min=0)],
...
class GrantForm(FlaskForm):  
    ...
    organism = StringField('Organism (funding succeptible) :*', validators=[DataRequired(), Length(min=2)],render_kw={'placeholder': 'Structure or organization funding the project'})
    principal_investigator = StringField('Principal investigator :*', validators=[DataRequired(), Length(min=2)],render_kw={'placeholder': 'first and last name'})
    promotor = StringField('Promotor :*', validators=[DataRequired(), Length(min=2)],render_kw={'placeholder': 'promotor(s) names (ex: APHP, DRCI)'})
    funding_type = StringField('funding_type :*', validators=[DataRequired(), Length(min=2)],render_kw={'placeholder': 'funding_type(s) names (ex: APHP, DRCI)'})
    total_amount = FloatField('total_amount :*', validators=[DataRequired(), Length(min=2)],render_kw={'placeholder': 'total_amount'})
    deadline = DateField('deadline :*', validators=[DataRequired(), Length(min=2)],render_kw={'placeholder': 'deadline'})
    urgency_of_request = SelectField('Urgency of request :*', choices=[('Not urgent', 'Not urgent'),
                                                                      ('Not particularly urgent', 'Not particularly urgent'),
                                                                      ('Very urgent', 'Very urgent')
                                                                      ])
    if_urgency = TextAreaField('If very urgent, briefly explain the issues :*',render_kw={'placeholder': 'Explain the reason for the emergency, depending on what has been selected from the drop-down list'})
    ...
    data_owner = StringField('Data owner :', validators=[DataRequired()],render_kw={'placeholder': 'Qui detient les donnees?'})
    regulatory_requirements = BooleanField('Regulatory requirements are available?*')
    if_regulatory_requirements = SelectField('If yes, precise : ', choices=[('CPP', 'CPP'),
                                                                            ('Ethical Comity', 'Ethical Comity'),
                                                                            ('Jarde law conformity', 'Jarde law conformity'),
    ...
    data_type = StringField('Data type :*', validators=[DataRequired(), Length(min=2)],render_kw={'placeholder': 'ex: fastq, tables de comptage, bam, etc.'})
